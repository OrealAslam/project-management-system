...
<div class="col-md-6 offset-md-3 my-4">
    ...
    <div class="card-body shadow bg-light">
        <form action="{{ route('register.admin') }}" method="post">
            ...
            <input type="text" class="form-control mb-3" name="adminName" id="name" placeholder="Name">
            <!-- display error (if any) -->
            @if($errors->any())
            @foreach($errors->get('adminName') as $name_err)
            ...
            @if($errors->any())
            @foreach($errors->get('adminEmail') as $email_err)
            <small class="text-danger ml-3">{{ $email_err }}</small>
            @endforeach
            @endif
            <br>
            <label class="form-label text-muted" for="password">Password</label>
            <input type="password" class="form-control" name="adminPassword" id="password" placeholder="Password">
            @if($errors->any())
            @foreach($errors->get('adminPassword') as $pass_err)
